...
import { DatabaseConnectionException, Handler, NotFoundException } from "../exceptions"
...
const SwaggerDocument = JSON.parse(fs.readFileSync(path.join(process.cwd(), env('SWAGGER_DOCUMENT_PATH')), 'utf-8'))
...
class ExpressServer extends BaseServer<express.Application> {
    ...
    private routes() {
        ...
        this.server.use('/v1/api-docs', swaggerUi.serve, swaggerUi.setup(SwaggerDocument, { explorer: true }))
